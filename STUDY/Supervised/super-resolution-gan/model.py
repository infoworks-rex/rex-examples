...
def _dense_block(x,in_layer=64,out_layer=64, beta=5):
    ...
    #h6 = tf.scalar_mul(beta, h5)
    return h5 / beta + x

def _RRDB(x,in_layer=64,hidden=32,name="rrdb", beta=5):
    with tf.variable_scope(name):
        ...
        with tf.variable_scope('block3'):
            out = _dense_block(db)
            #out = tf.scalar_mul(beta, out)
    return out / beta + x

def buildESRGAN_g(x,reuse=False,isTraining=True, beta=5):
