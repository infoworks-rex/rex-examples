...
#define MODEL_NAME	"/mySSD/trafficlight.rknn"
#define PRIOR_BOX 	"/usr/share/rknn_demo/box_priors.txt"
#define LABELS		"/mySSD/label.txt"
...
void* ssd_loop(void* ptr)
{
	...
	ssd_init(MODEL_NAME, PRIOR_BOX, LABELS, &ctx);
	printf("init rknn complete \n");
	while (1) {
		rga_set_rect(&r_src.rect, 0, 0, CAM_W, CAM_H, CAM_W, CAM_H, RK_FORMAT_YCrCb_420_P);
		rga_set_rect(&r_dst.rect, 0, 0, 300, 300, 300, 300, RK_FORMAT_RGB_888);
		ret = c_RkRgaBlit(&r_src, &r_dst, NULL);

		ssd_run(&ctx, (uint8_t*)dst_v, 300, 300, 270000);
