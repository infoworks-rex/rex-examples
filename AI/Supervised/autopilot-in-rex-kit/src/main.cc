...
#define MODEL_NAME	"/autopilot.rknn"
uint8_t img_buf[200*112*3];
...
{	
	...
	FILE *image = fopen("/raw0.raw", "r");
	status = fread(img_buf, sizeof(unsigned char), 200*112*3, image);
	fclose(image);
	while(1){
		rga_set_rect(&r_src.rect, 0, 0, CAM_W, CAM_H, CAM_W, CAM_H, RK_FORMAT_YCrCb_420_P);
		rga_set_rect(&r_dst.rect, 0, 0, 200, 112, 200, 112, RK_FORMAT_RGB_888 );
		ret = c_RkRgaBlit(&r_src, &r_dst, NULL);

		ssd_run(&ctx, (uint8_t*)dst_v, 200, 112, 200* 112* 3);

		//printf(" detections count %d , %d \n ",ctx.detections.count, ctx.detections.objects[0].rect.left);
	}
